...
export class LidarrAdapter extends ApplicationApiAdapter {
    ...
    static canHandle(containerName: string, image: string): boolean {
        ...
        return lowerName.includes('lidarr') || lowerImage.includes('lidarr');
    }
    ...
    protected getApiUrl(): string {
        ...
        // Default to localhost with common Lidarr port
        return `http://${this.containerName}:8686/api/v1/update`;
    }

    protected formatReleaseNotes(updates: LidarrUpdate[]): string {
    ...
    async fetchUpdateInfo(): Promise<AppUpdateInfo | null> {
        try {
            ...
            logger.debug(`Fetching Lidarr updates from: ${url}`);
            
            const response = await this.http.get<LidarrUpdate[]>(url, { headers });
            
            if (!response.data || response.data.length === 0) {
                logger.warn(`No update information available for Lidarr container: ${this.containerName}`);
                return null;
            }
            ...
            logger.info(`Successfully fetched Lidarr update info for ${this.containerName}`);
            ...
            if (error.code === 'ECONNREFUSED' || error.code === 'ETIMEDOUT') {
                logger.debug(`Could not connect to Lidarr API for ${this.containerName}: ${error.message}`);
            } else if (error.response?.status === 401) {
                logger.warn(`Lidarr API authentication failed for ${this.containerName}. API key may be required.`);
            } else {
                logger.debug(`Failed to fetch Lidarr update info for ${this.containerName}: ${error.message}`);
            }
            return null;
        }
    }
