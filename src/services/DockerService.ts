...
export default class DockerService {
   ...
   */
  public static async getImageNewDigestWithReleaseNotes(imageName: string, tag: string, installedVersion?: string): Promise<{ newDigest: string | null; releaseNotes?: string; repoUrl?: string; }> {
    try {
      let adapter = ImageRegistryAdapterFactory.getAdapter(imageName, tag, undefined, installedVersion);
      let response = await adapter.checkForNewDigest();

      return { newDigest: response.newDigest, releaseNotes: response.releaseNotes, repoUrl: response.repoUrl };
